...
router.get('/bureau/:id', async (req, res) => {
  const { id: bureauId } = req.params; // Extract `bureauId` from URL parameters

  try {
    // Find the user by the bureauId field
    const user = await User.findOne({ bureauId }); // Use `findOne` with a filter condition
...
router.get('/bureau/:bureauId+/:gender', async (req, res) => {
  ...
  try {
    ...
    const users = await User.find({ gender, bureauId }); // Use `find` to get multiple documents
...
router.get('/gender/female', async (req, res) => {
  try {
    // Find the user by gender "male"
    const user = await User.find({ gender: "female" }); // Use `findOne` with a fixed filter condition
    
    // If no user with gender "male" is found, return a 404 response
    if (!user) {
      return res.status(404).json({ message: 'FeMale user not found' });
    }
...
router.get('/gender/male', async (req, res) => {
  try {
    // Find the user by gender "male"
    const user = await User.find({ gender: "male" }); // Use `findOne` with a fixed filter condition
